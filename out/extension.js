...
async function findConfigurationFiles(config) {
...
async function backupConfigurations(configPaths) {
...
async function updateConfigurations(configPaths, protocolFilePath) {
...
async function updateServerConfig(serverConfigPath, protocolFilePath) {
    try {
...
async function updateClientConfig(clientConfigPath) {
    try {
...
async function startServerApplication(configPaths, protocolFile) {
    ...
    serverProcess = (0, child_process_1.spawn)(executablePath, [], {
...
async function startClientApplication(configPaths, protocolFile) {
    ...
    clientProcess = (0, child_process_1.spawn)(executablePath, [], {
...
function showTestControlPanel(protocolFile, configPaths) {
    const panel = vscode.window.createWebviewPanel('testControlPanel', `Test Control - ${protocolFile}`, vscode.ViewColumn.Two, { enableScripts: true });
...
async function cleanupTest(configPaths) {
    await stopRunningTest();
}
// ============================================================================
...
async function configureTestPaths() {
    ...
    const serverDir = await vscode.window.showInputBox({
        ...
        validateInput: (value) => {
            if (!value)
                return 'Server directory cannot be empty';
            return null;
        }
    });
    if (!serverDir)
    ...
    const clientDir = await vscode.window.showInputBox({
        ...
        validateInput: (value) => {
            if (!value)
                return 'Client directory cannot be empty';
            return null;
        }
    });
    if (!clientDir)
...
async function validateProtocolFileInternal(document, showMessages = false) {
    try {
        ...
        if (!valid && validate.errors) {
            ...
            if (showMessages) {
                const action = await vscode.window.showErrorMessage('❌ Validation failed. Check Problems panel.', 'Show Problems');
...
function findLineForPath(content, path) {
...
async function validateProtocolFileAuto(document, showSuccessMessage = false) {
...
async function createQuickTemplate(templateType) {
    try {
...
function generateTestDataScenario(scenario) {
    const baseTemplate = {
        "$schema": "./schemas/testDataProtocolFile.schema.json",
        "SimulatedData": []
    };
...
async function runTestWithProtocol() {
    ...
    const action = await vscode.window.showInformationMessage('🚀 Choose test runner method:', 'Integrated Test Runner', 'Legacy Batch Script');
...
async function runLegacyBatchScript() {
    ...
    try {
        ...
        if (!testScript) {
            const action = await vscode.window.showErrorMessage(`🔍 No test script found in: ${protocolDir}`, 'Create Test Script', 'Continue Anyway');
        ...
        const confirmed = await vscode.window.showInformationMessage(`🚀 Run test "${scriptName}" with "${protocolName}"?`, 'Run Test', 'Cancel');
        if (confirmed !== 'Run Test')
            return;
        ...
        terminal.show();
        vscode.window.showInformationMessage(`⚡ Test started! Check terminal for progress.`);
    }
    catch (error) {
        vscode.window.showErrorMessage(`💥 Error running test: ${error}`);
    }
}
async function createTestScript(dir) {
...
`;
    ...
    try {
        ...
        const action = await vscode.window.showInformationMessage(`✅ Test script created: ${scriptPath}`, 'Open Script', 'Run Now');
...
async function showWelcomeMessage(context) {
    ...
    if (!hasShownWelcome) {
        const action = await vscode.window.showInformationMessage('🎉 ProtocolFile Extension activated! Ready to boost your testing workflow.', 'Show Help', 'Create First Protocol', 'Don\'t Show Again');
        if (action === 'Show Help') {
            await showHelpPanel();
        }
...
function getTestControlHTML(protocolFile) {
    return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
